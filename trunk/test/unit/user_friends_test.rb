...
#  this test shows that you can all user's excluding this user
  def test_addFriends
      bFound = false;
      user1 = User.new(:username=>'forest1', :password=>"gump123", :start_year=>'2009', :start_sem=>'FA', :birthday=>'1990-10-29', :college => College.find(:first))
      user2 = User.new(:username=>'forest2', :password=>"gump234", :start_year=>'2007', :start_sem=>'FA', :birthday=>'1930-10-29', :college => College.find(:first))
      user1.save
      user2.save
      @friends_all = User.find :all, :order => 'last_name ASC, first_name ASC'  
